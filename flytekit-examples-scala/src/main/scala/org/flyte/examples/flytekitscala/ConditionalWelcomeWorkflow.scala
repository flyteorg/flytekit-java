...
import org.flyte.flytekitscala.{SdkScalaType, SdkScalaWorkflow, SdkScalaWorkflowBuilder}
...
class WelcomeWorkflow
  ...
  override def expand(
    ...
    val greeting = builder.apply(
        SdkConditions.when(
            "when-empty",
            SdkConditions.eq(input.name, SdkBindingDataFactory.of("")),
            new GreetTask(),
            GreetTaskInput(SdkBindingDataFactory.of("World"))
          )
          .otherwise("when-not-empty", new GreetTask(), GreetTaskInput(input.name)))
      .getOutputs
      .greeting
